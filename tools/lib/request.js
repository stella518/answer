...
var buildSessID = function buildSessID(session) {
  
  var data = {};

  if (session && session.sessID) {
      data.sessID = session.sessID
  }
  return data;
};
...
function request(options) {
    ...
    function doRequest() {
        ...
        var sessID = buildSessID(Session.get());
        var sendata = utils.extend({}, sessID, options.data);

        options.data = sendata;
